...
def get_json():
    ...
    w_value = float(request.form['Weight value'])
    w_unit = str(request.form['Unit of weight'])
    h_value = float(request.form['height value'])
    h_unit = str(request.form['Unit of height'])
    firstName = Fname[0].upper() + Fname[1:].lower()
    lastName = Lname[0].upper() + Lname[1:].lower()
    name = f'{firstName} {lastName}'
    weight = f'{w_value}{w_unit}'
    height = f'{h_value}{h_unit}'
    Employee = [name, weight, height]
    my_json = dict(zip(header, Employee))
    return my_json
